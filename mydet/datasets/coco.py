...
class CocoDataset(BaseDetDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        toy_dataset = ToyDataset(
                            ata_root='data/',
                            ata_prefix=dict(img_path='train/'),
                            nn_file='annotations/train.json',
                            pipeline=pipeline)
        """
        with get_local_path(
    ...
    # override parse_data_info
    def parse_data_info(self, raw_data_info: dict) -> Union[dict, List[dict]]:
        """
        解析标注文件里的每个原始数据
        coco目标框格式：矩形框左上角的坐标(x1, y1)和矩形框的长宽(w, h)
        数据集bbox存储格式：左上角坐标(x1, y1), 右下角坐标(x1 + w, y1 + h)
        """
        ...
        for i, ann in enumerate(ann_info):
            ...
            instance['bbox'] = bbox
            instance['bbox_labeel'] = self.cat2label[ann['category_id']]
