...
class ClassAwareSampler(Sampler):
    ...
    """
    类感知采样器 应对长尾问题
    使用了两个 list 来解决：一是 class list (List) 用于统计类别数据；二是 
    per-class image lists (List[List])，用来保存对应的类别在哪些图片中出
    现。
    具体流程：首先从 class list 中抽取一个类别 X，然后从 per-class image 
    lists 对应 X 类别的 images list 中抽取 1 张图片。当对应 X 类别出现的
    图片已经取完之后对这个列表进行打乱。
    """
